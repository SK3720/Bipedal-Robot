...
class Cfg:
    ...
    swing_ms: int = 200
    swing_hip_rad: float = 0.46
    swing_knee_peak: float = 0.20
    swing_knee_land: float = 0.05     # knee flex retained at footstrike (near-straight)
    swing_ankle_dorsi: float = 0.09
    swing_lift_hold_ms: int = 30      # extra ankle-roll hold to keep foot unloaded through lift
    ...
    # terminal descent
    descend_ms: int = 70
    plant_hip_retract: float = 0.12
    plant_ankle_pf: float = 0.34
    plant_ankle_flat_gain: float = 1.0   # ankle pitch per rad of torso fwd-lean at touchdown
...
def run(
    ...
    while k < T_END:
        ...
        if phase in ("stand", "settle"):
            ...
            # REAL step: freed leg genuinely lifts (knee bends for clearance),
            # swings forward, knee re-extends and the ankle actively flattens the
            # sole for touchdown.  The StandingLQR still does ALL of the sagittal
            # balance (no brake, no push-off, no hip extension); the step rides
            # the push's own forward momentum onto the new foot.
            if not swing_started:
                hip = kn = an = 0.0
            else:
                s = sk - swing_t0
                sw_hip = cfg.swing_hip_rad * amp_scale
                if s < cfg.swing_ms:  # lift + swing forward with knee clearance
                    w = s / cfg.swing_ms
                    hip = hf * sw_hip * _smooth(w)
                    kn = KNEE_FLEX * (
                        cfg.swing_knee_land
                        + (cfg.swing_knee_peak - cfg.swing_knee_land) * np.sin(np.pi * w)
                    )
                    an = -hf * cfg.swing_ankle_dorsi * np.sin(np.pi * w)
                else:  # descend: retract hip slightly, keep knee near-straight,
                    # actively pitch the ankle to hold the sole flat vs the
                    # (forward-leaning) torso so it lands flat, not toe- or
                    # heel-first.
                    w = min(1.0, (s - cfg.swing_ms) / cfg.descend_ms)
                    sm = _smooth(w)
                    hip = hf * (sw_hip - cfg.plant_hip_retract * sm)
                    kn = KNEE_FLEX * cfg.swing_knee_land
                    flat = np.radians(bs.fwd_lean_deg) * cfg.plant_ankle_flat_gain
                    an = (
                        -hf * cfg.swing_ankle_dorsi * (1 - sm) * 0.5
                        + hf * flat * sm
                    )
