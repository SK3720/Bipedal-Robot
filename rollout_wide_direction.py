"""Visual rollout for wide-direction recovery policy with slow-motion around push."""
...
SAVE_DIR = "ppo_poc_wide_direction"
...
CASES = {
    ...
    "120N +Y": {"push_magnitude": 120.0, "push_direction_rad": np.pi / 2},
    "120N -Y": {"push_magnitude": 120.0, "push_direction_rad": 3 * np.pi / 2},
}
...
def rollout(case_name, case, seed=0):
    ...
    print(
        f"\nWide-direction rollout: {case_name} | push {reset_info['push_magnitude']:.1f} N "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Wide-direction recovery rollout")
    parser.add_argument("--case", default=DEFAULT_CASE, help="Case name")
...
def main():
    ...
    if args.case not in CASES:
        raise SystemExit(f"Unknown case '{args.case}'. Use --list-cases.")
