"""PPO for full-authority reactive sequential push recovery (biped_walk_env).

Policy: 10-joint position residual on the StandingLQR + whip-retract + frontal/
lateral-LIPM base, canonical "swing = R" frame.  Zero-init action head -> starts
at the (already partly working) reference gait.  Fresh policy (new obs/action).

  python train_walk_ppo.py --steps 300000 --envs 8 --tag w0_smoke
  python train_walk_ppo.py --resume runs/walk_w1 --steps 2000000

Checkpoints: policy.pth + vecnormalize.pkl + hparams.json.
"""
...
INFO_KW = ("success", "flat_ok", "n_steps", "peak_up", "fell", "end_spd",
           "end_capt_fwd_mm", "push_n", "timed_out")

# curriculum: (push_lo, push_hi, dir_spread_deg).  Start in the zone the reference
# gait already handles, widen out + up.
CURRICULUM = [
    (122.0, 144.0, 0.0),
    (121.0, 146.0, 5.0),
    (120.0, 148.0, 8.0),
...
class EvalCB(BaseCallback):
    ...
    def _init(self):
        ...
        self._wide = BipedWalkEnv(model_path=self.model_path, push_band=(120.0, 150.0),
                                  dir_spread_deg=14.0)

    def _run(self, env, n, collect=0):
        vn = self.model.get_vec_normalize_env()
        succ = flat = fell = tmo = 0
        steps, ups, spds = [], [], []
        frames = []
        for i in range(n):
            ...
            if i < collect:
                frames.append(ep)
        return dict(sr=succ / n, flat=flat / n, fell=fell / n, tmo=tmo / n,
                    steps=float(np.mean(steps)), up=float(np.median(ups)),
                    spd=float(np.median(spds)),
                    dist=np.bincount(steps, minlength=6).tolist(), frames=frames)
    ...
    def _on_step(self):
        ...
        if self.verbose:
            ...
            print(f"  [eval @ {self.num_timesteps}] band {b} +-{s:.0f}  "
                  f"SUCC {r['sr']:.2f} (flat {r['flat']:.2f}, wide {w['sr']:.2f})  "
                  f"fell {r['fell']:.2f}  timeout {r['tmo']:.2f}")
            print(f"      steps~{r['steps']:.1f} dist {r['dist'][1:]}  peakUp {r['up']:.1f}  endSpd {r['spd']:.2f}")
        # best = training-band success + wide-band success + a bonus for flat finishes
        score = 0.55 * r["sr"] + 0.30 * w["sr"] + 0.15 * r["flat"] - 0.5 * r["fell"]
